...
def main(submatfile,seq1,seq2,gapopen=-11,gapext=-1,
    road='high',js=False,fm=False):
    '''
    ...
    scoremat, dirmat, start_pos, glob_max_score = create_scoremat(submat, rows, cols, seq1, 
                                                  seq2, gapopen, gapext, road, js)
    if js:
        glob_max_score = scoremat # Maxscore was passed as scoremat
        return glob_max_score
    ...
    for i in range(0, alength, 60):
        ...
        print('')
    return seq1_aligned, seq2_aligned, glob_max_score

def create_scoremat(submat, rows, cols, seq1, seq2, 
                    gapopen, gapext, road, js):
    '''
    ...
    scorematlow = [[0 for col in range(cols)] for row in range(rows)]
    scorematmid = [[0 for col in range(cols)] for row in range(rows)]
    scorematup = [[0 for col in range(cols)] for row in range(rows)]
    
    scorematlow[0] = [0] + [-np.inf for _ in range(cols - 1)]
    for i in scorematlow[1:]:
        i[0] = -np.inf

    scorematup[0] = [0] + [-np.inf for _ in range(cols - 1)]
    for i in scorematup[1:]:
        i[0] = -np.inf

    scoremat = np.stack((scorematlow,scorematmid,scorematup))
    ...
    for i in range(1, rows):
        for j in range(1, cols):
            ...
            score, dirmat = calc_score(seq1, seq2, scoremat, submat, dirmat, 
                                       gapopen, gapext, i, j, road)
            # The max score is where we will start the traceback
            if score > glob_max_score:
                glob_max_score = score
                max_pos   = (i, j)
            # Fill in the scoremat with the score found for the given position
            scoremat[1][i][j] = score
    # In case, for some reason, you didn't find a score > 0:
    assert max_pos is not None, 'The x, y position with the highest score was not found.'
    if js:
        return glob_max_score, None, None, None
    return scoremat[1], dirmat, max_pos, glob_max_score

def calc_score(seq1, seq2, scoremat, submat, dirmat, gapopen, 
               gapext, x, y, road='low'):
    '''
    ...
    # Lower matrix
    lowlow = scoremat[0][x-1][y] + gapext
    lowmid = scoremat[1][x-1][y] + gapopen + gapext
    scoremat[0][x][y] = max(lowmid, lowlow)

    # Upper matrix
    upmid = scoremat[1][x][y-1] + gapopen + gapext
    upup = scoremat[2][x][y-1] + gapext
    scoremat[2][x][y] = max(upmid, upup)

    # Middle matrix
    midlow = scoremat[0][x][y]
    midmid = scoremat[1][x-1][y-1] + similarity
    midup = scoremat[2][x][y]
    scores = (0, midlow, midmid, midup)
